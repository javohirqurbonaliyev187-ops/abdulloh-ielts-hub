...
const skillModules = [
  {
    ...
    features: [
      "40+ Practice Tests with Transcripts",
  ...
  },
  {
    ...
    features: [
      "Task 1: Reports, Letters, Graphs",
      "Task 2: Essay Writing",
      "AI Grammar & Structure Feedback",
        ...
        <section className="section-padding bg-gradient-to-b from-background to-muted/30">
          <div className="container-main text-center">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto mb-8">
              Choose a skill module to start practicing. Each module includes comprehensive 
              practice materials, strategies, and AI-powered feedback.
            </p>
              ...
              <div className="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className={`space-y-6 ${index % 2 === 1 ? "lg:order-2" : ""}`}>
                  ...
                  <div className="flex gap-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        <strong>{skill.stats.tests}</strong> Tests
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className={`w-5 h-5 text-${skill.color}`} />
                      <span className="text-sm">
                        <strong>{skill.stats.minutes}</strong> min each
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Trophy className={`w-5 h-5 text-${skill.color}`} />
                      <span className="text-sm">
                        Avg: <strong>{skill.stats.avgScore}</strong>
                      </span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-3">
                    {skill.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3">
                        <CheckCircle className={`w-5 h-5 text-${skill.color}`} />
                  ...
                  <div className={`p-4 rounded-lg bg-${skill.color}/5 border border-${skill.color}/20`}>
                    <p className="text-sm">
                      <strong className={`text-${skill.color}`}>Pro Tip:</strong>{" "}
                      <span className="text-muted-foreground">{skill.tips}</span>
                    </p>
                  </div>
                  ...
                  <div className={`card-elevated p-8 border-2 border-${skill.color}/20`}>
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      {[1, 2, 3, 4].map((item) => (
                        <div
                          key={item}
                          className={`aspect-square rounded-lg bg-${skill.color}/5 flex items-center justify-center`}
                        >
                          <div className={`w-12 h-12 rounded-full bg-${skill.color}/10 flex items-center justify-center`}>
                            <span className={`text-lg font-bold text-${skill.color}`}>
                              {item}
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="text-center">
                      <p className={`text-sm font-medium text-${skill.color}`}>
                        Test Sections
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Complete all sections to unlock advanced tests
                      </p>
                    </div>
                  </div>
                </div>
